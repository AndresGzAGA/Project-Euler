...
What is the greatest product of four adjacent numbers in the same direction (up, down, left, right, or diagonally) in the 20×20 grrid?
'''
...
def N(x,y):
    '''
    It returns the number given by the coordinates (x,y)
    1<=x=<20
    1<=y=<17
    #Coordinate translated to number in the string g: n(x,y)=20*(y-1)+x
    #Number in the string g translated into the place in which the first digit of the dumber resides: s(n)=3*(n-1)=3*n-3 second digit: s(n)=3*(n-1)+1=3*n-2
    #First digit: s(x,y)=60*(y-1)+3*(x-1)   Second digit: s(x,y)+1
    '''
    if g[60*(y-1)+3*(x-1)]==0: #To avoid a number like 08
        return int(g[60*(y-1)+3*(x-1)+1])
    return int(g[60*(y-1)+3*(x-1):60*(y-1)+3*(x-1)+2])


def Mv(x,y):
    '''
    Multiplies the number given by the coordinates x,y in the grid and the 3 numbers vertically below him.
    1<=x=<20
    1<=y=<17
    '''
    return N(x,y)*N(x,y+1)*N(x,y+2)*N(x,y+3)
    
def Mh(x,y):
    '''
    Multiplies the number given by the coordinates x,y in the grid and the 3 numbers horizontally to the right of him.
    1<=x=<17
    1<=y=<20
    '''
    return N(x,y)*N(x+1,y)*N(x+2,y)*N(x+3,y)
    
def Mdr(x,y):
    '''
    Multiplies the number given by the coordinates x,y in the grid and the 3 numbers in a diagonal line to the right and below him.
    1<=x=<17
    1<=y=<17
    '''
...
def Mdi(x,y):
    '''
    Multiplies the number given by the coordinates x,y in the grid and the 3 numbers in a diagonal line to the left and below him.
    4<=x=<20
    1<=y=<17
    '''
    return N(x,y)*N(x-1,y+1)*N(x-2,y+2)*N(x-3,y+3)
